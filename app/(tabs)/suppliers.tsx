...
import { useDeleteSupplier, useSuppliersPage } from "@/lib/hooks/useSuppliers";
...
export default function SuppliersScreen() {
  ...
  const { data, isLoading, isRefetching, refetch, error } = useSuppliersPage(1, {
  ...
  const onLongPress = (item: Supplier) => {
    ...
    Alert.alert(item.name, undefined, [
      { text: t("cancel"), style: "cancel" },
      {
        ...
        style: "destructive",
        onPress: () =>
          Alert.alert(t("delete_supplier_title"), t("delete_supplier_body"), [
            { text: t("cancel"), style: "cancel" },
            {
              text: t("delete"),
              style: "destructive",
    ...
    return (
      <View style={styles.flex}>
        ...
        <SafeAreaView style={styles.safe} edges={["top"]}>
          <ScreenHeroHeader
            eyebrow={t("suppliers_eyebrow")}
            title={t("suppliers_title")}
            subtitle={t("suppliers_sign_in_prompt")}
    ...
    <View style={styles.flex}>
      ...
      <View style={[styles.fabWrap, { bottom: fabBottomOffset }]} pointerEvents="box-none">
        <Pressable
          ...
          accessibilityLabel={t("add_supplier")}
          style={({ pressed }) => [{ opacity: pressed ? 0.9 : 1 }]}
        >
          <LinearGradient
            colors={[BakimateColors.accentTeal, BakimateColors.primary]}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 1 }}
            style={styles.fab}
          >
      ...
      <AddSupplierSheet visible={addOpen} isDark={isDark} onClose={() => setAddOpen(false)} />
    </View>
  );
}
